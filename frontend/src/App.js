...
function NavBar({ onLoginClick, scrollTo }){
  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-[#0a2a52] text-white z-20">
      <div className="max-w-6xl mx-auto flex items-center justify-between p-4">
        <div className="font-bold">Midway Mobile Storage</div>
        ...
        <div className="md:flex items-center hidden">
          <button onClick={onLoginClick} className="bg-[#e84424] px-3 py-1 rounded">Admin</button>
        </div>
        <button className="md:hidden" onClick={()=>setOpen(!open)}>{open ? <X/> : <Menu/>}</button>
      </div>
      {open && (
        <div className="md:hidden bg-[#0d3464] p-4">
          <button className="block w-full text-left" onClick={() => {scrollTo('hero'); setOpen(false)}}>Home</button>
          <button className="block w-full text-left" onClick={() => {scrollTo('services'); setOpen(false)}}>Services</button>
        </div>
      )}
    </header>
  )
